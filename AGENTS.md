...
- `POST /api/hydra-admin/clients/[id]/regenerate-secret` — регенерация секрета
...
- Два URL Hydra в test-app: `HYDRA_INT_URL=http://hydra:4444` (сервер) / `HYDRA_BROWSER_URL=http://localhost:4444` (редиректы).
...
## Инфраструктура
- `docker-compose.yml` — 10 сервисов (вкл. миграционные), resource limits, healthchecks
